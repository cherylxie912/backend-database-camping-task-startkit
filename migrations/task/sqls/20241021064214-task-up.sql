...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
    and booking_at  = '2024-11-24 17:10:25.000';

-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)

    select user_id, sum(purchased_credits) as toral
...
-- 5-8. [挑戰題] 查詢：請在一次查詢中，計算用戶王小明的剩餘可用堂數，顯示須包含以下欄位： user_id , remaining_credit
    ...
    from (select user_id, sum(purchased_credits) AS total_credit
        from "USER"
        inner join "CREDIT_PURCHASE" cp  on "USER".id = user_id
...
-- 顯示須包含以下欄位： 總營收
    ...
    where extract (month from "CREDIT_PURCHASE".created_at) = 12;
...
-- 顯示須包含以下欄位： 預約會員人數
    ...
    from "COURSE_BOOKING"
    where extract (month from "COURSE_BOOKING".created_at) = 12
    and status != '課程已取消';
